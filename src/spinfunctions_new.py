...
class SpinFunctions:
    ...
    def get_sigma2(self):
        ...
        def Sn_spin(mass, Sn, E1, a, Ed, σ_d, Ex = self.Ex): 
            σ_sq_Sn = 0.0146 * mass**(5.0/3.0) * (1 + np.sqrt(1 + 4*a*(Sn - E1))) / (2*a)
            σ_sq_Ex = σ_d**2 + (Ex - Ed) / (Sn - Ed) * (σ_sq_Sn - σ_d**2)
        ...
        if model == "EB05":
            ...
            pars_req = {"mass", "Sn", "E1", "a", "Ed", "σ_d"}
            return call_model(Sn_spin, pars, pars_req)

...
from spinfunctions_new  import SpinFunctions
...
def log_interp1d(xx, yy, **kwargs):
    ...
    logy = np.log(np.clip(yy, 1e-20, None))
...
def gen_nld_table(fnld, Estop, model, spinpars, A):
    ...
    if A % 2 == 1:
        Js += 0.5
    spindist = SpinFunctions(E, Js, model=model, pars=spinpars).distribution()
    spindist /= spindist.sum(axis=1)[:, np.newaxis]

    # fake temperature
    T_dummy = np.full_like(E, 0.915)
...
if __name__ == "__main__":
    ...
    spinpars = {"mass": A, "Sn": Sn, 'E1': 1.12, 'a': 13.92, 'Ed': 2.53, 'σ_d': 3.4, 'Ex': None}

    # load/write nld
    # fn_nld = "nld_exp.txt"
    # fn_nld_out = "nld_talys.txt"
    ...
    # fnld = log_interp1d(nld[:, 0], nld[:, 1], fill_value="extrapolate", bounds_error=False)


    # # print(f"Below {nld[0, 0]} the nld is just an extrapolation
    # #       "Best will be to use discrete levels in talys below that")
    # try:
    #     table = gen_nld_table(fnld=fnld, Estop=Estop, model="Disc_and_EB05",
    ...
    fn_gsf = "gsf_exp.txt"
    fn_gsf_outE1 = f"gsfE1_{Z}_{A}.dat"
    fn_gsf_outM1 = f"gsfM1_{Z}_{A}.dat"
    ...
    try:
        talys_out = np.loadtxt(f"data/talys_output_{Z}_{A}.txt", skiprows=2)
